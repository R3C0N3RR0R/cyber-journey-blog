...
- **Escalade de privilèges** : Écrire dans des fichiers protégés
- **Bypass de restrictions** : Contourner les contrôles d'accès
- **Race conditions** : Exploiter des fenêtres temporelles
...
# Pourquoi : Pointe vers un chemin, peut traverser les systèmes de fichiers
ln -s /target/file /link/to/file

# Lien dur (hard link)
# Pourquoi : Pointe vers l'inode, même système de fichiers uniquement
...
#### Scénario
Un service privilégié écrit dans un fichier sous notre contrôle.
...
#### Contexte
Le service "Below" écrit ses logs avec des privilèges root dans un répertoire world-writable.
...
#### Technique
Exploiter une fenêtre temporelle entre vérification et utilisation.
...
### Prévention côté système

```bash
# 1. Restrictions de permissions
# Pourquoi : Empêcher la création de symlinks malveillants
...
*Les attaques par symlink démontrent l'importance de la validation rigoureuse des chemins de fichiers et de la gestion sécurisée des permissions dans les systèmes Unix/Linux.*